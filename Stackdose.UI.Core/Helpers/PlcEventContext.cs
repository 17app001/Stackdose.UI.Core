...

{
...

    {
        ...
        /// <summary>
        /// 控件值變更事件（由 PlcControlBase.RaiseValueChanged 推送至此）
        /// B5 Behavior Engine 訂閱此事件做條件比對
        /// </summary>
        public static event EventHandler<PlcValueChangedEventArgs>? ControlValueChanged;
        ...
        /// <summary>
        /// 由 PlcControlBase.RaiseValueChanged 呼叫，將控件值變更推送到統一匯流排
        /// </summary>
        public static void PublishControlValueChanged(object sender, PlcValueChangedEventArgs args)
        {
            if (args == null) return;
            Application.Current?.Dispatcher.BeginInvoke(() =>
            {
                ControlValueChanged?.Invoke(sender, args);
            });
        }
