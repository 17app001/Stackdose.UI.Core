...

{
...

    {
        ...
        /// 匯出操作日誌到 PDF（文字格式）
        /// </summary>
        public void ExportOperationLogsToPdf(List<OperationLogRecord> records, string filePath)
        {
            using var writer = new System.IO.StreamWriter(filePath, false, System.Text.Encoding.UTF8);
            writer.WriteLine("=".PadRight(100, '='));
            writer.WriteLine($"{"操作日誌報表 (Operation Log Report)",60}");
            writer.WriteLine($"{"匯出時間",20}: {DateTime.Now:yyyy-MM-dd HH:mm:ss}");
            writer.WriteLine($"{"記錄數量",20}: {records.Count} 筆");
            writer.WriteLine("=".PadRight(100, '='));
            writer.WriteLine();
            ...
            {
                writer.WriteLine("-".PadRight(100, '-'));
                writer.WriteLine($"{"ID",15}: {r.Id}");
                writer.WriteLine($"{"時間戳記",15}: {r.Timestamp:yyyy-MM-dd HH:mm:ss.fff}");
                writer.WriteLine($"{"使用者",15}: {r.UserId}");
                writer.WriteLine($"{"批次 ID",15}: {r.BatchId}");
                writer.WriteLine($"{"指令名稱",15}: {r.CommandName}");
                writer.WriteLine($"{"類別",15}: {r.Category}");
                writer.WriteLine($"{"執行前狀態",15}: {r.BeforeState}");
                writer.WriteLine($"{"執行後狀態",15}: {r.AfterState}");
                writer.WriteLine($"{"訊息",15}: {r.Message}");
                writer.WriteLine();
            }
            ...
            writer.WriteLine($"{"報表結束",60}");
            writer.WriteLine("=".PadRight(100, '='));
            System.Diagnostics.Debug.WriteLine($"[LogService] Exported {records.Count} operation logs to {filePath}");
        }

        /// <summary>
        /// 匯出事件日誌到 PDF（文字格式）
        /// </summary>
        public void ExportEventLogsToPdf(List<EventLogRecord> records, string filePath)
        {
            using var writer = new System.IO.StreamWriter(filePath, false, System.Text.Encoding.UTF8);
            writer.WriteLine("=".PadRight(100, '='));
            writer.WriteLine($"{"事件日誌報表 (Event Log Report)",60}");
            writer.WriteLine($"{"匯出時間",20}: {DateTime.Now:yyyy-MM-dd HH:mm:ss}");
            writer.WriteLine($"{"記錄數量",20}: {records.Count} 筆");
            writer.WriteLine("=".PadRight(100, '='));
            writer.WriteLine();
            ...
            foreach (var r in records)
            {
                ...
                writer.WriteLine($"{"使用者",15}: {r.UserId}");
                writer.WriteLine($"{"批次 ID",15}: {r.BatchId}");
                writer.WriteLine($"{"事件類型",15}: {r.EventType}");
                writer.WriteLine($"{"事件代碼",15}: {r.EventCode}");
                writer.WriteLine($"{"嚴重程度",15}: {r.Severity}");
                writer.WriteLine($"{"目前狀態",15}: {r.CurrentState}");
                writer.WriteLine($"{"事件描述",15}: {r.EventDescription}");
                writer.WriteLine($"{"訊息",15}: {r.Message}");
                writer.WriteLine();
            }
            ...
            writer.WriteLine($"{"報表結束",60}");
            writer.WriteLine("=".PadRight(100, '='));
            System.Diagnostics.Debug.WriteLine($"[LogService] Exported {records.Count} event logs to {filePath}");
        }

        /// <summary>
        /// 匯出週期數據到 PDF（文字格式）
        /// </summary>
        public void ExportPeriodicDataLogsToPdf(List<PeriodicDataLogRecord> records, string filePath)
        {
            using var writer = new System.IO.StreamWriter(filePath, false, System.Text.Encoding.UTF8);
            writer.WriteLine("=".PadRight(100, '='));
            writer.WriteLine($"{"週期數據報表 (Periodic Data Log Report)",60}");
            writer.WriteLine($"{"匯出時間",20}: {DateTime.Now:yyyy-MM-dd HH:mm:ss}");
            writer.WriteLine($"{"記錄數量",20}: {records.Count} 筆");
            writer.WriteLine("=".PadRight(100, '='));
            writer.WriteLine();

            writer.WriteLine($"{"ID",-8} {"時間戳記",-25} {"使用者",-15} {"批次 ID",-15} {"預乾溫度",10} {"乾燥溫度",10} {"CDA 入口壓力",12}");
            writer.WriteLine("-".PadRight(100, '-'));

            foreach (var r in records)
            {
                writer.WriteLine($"{r.Id,-8} {r.Timestamp:yyyy-MM-dd HH:mm:ss.fff,-25} {r.UserId,-15} {r.BatchId,-15} {r.PredryTemp,10:F1} {r.DryTemp,10:F1} {r.CdaInletPressure,12:F2}");
            }
            ...
            writer.WriteLine($"{"報表結束",60}");
            writer.WriteLine("=".PadRight(100, '='));
            System.Diagnostics.Debug.WriteLine($"[LogService] Exported {records.Count} periodic data logs to {filePath}");
        }
