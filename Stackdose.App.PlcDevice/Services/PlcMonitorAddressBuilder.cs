...
public static class PlcMonitorAddressBuilder
{
    private static readonly Regex AddressPattern = new("^([A-Za-z]+)(\\d+)$", RegexOptions.Compiled);

    public static string Build(PlcMachineConfig config)
    {
        return Build([config]);
    }

    public static string Build(IEnumerable<PlcMachineConfig> configs)
    {
        var readTags = configs
    ...
    private static IEnumerable<string> ExpandAddresses(PlcTagConfig tag)
    {
    ...
    private static int GetAddressSpan(PlcTagConfig tag)
    {
        if (tag.Type.Equals("string", StringComparison.OrdinalIgnoreCase))
        {
            return Math.Max(1, tag.Length);
        }

        if (tag.Type.Equals("int32", StringComparison.OrdinalIgnoreCase) || tag.Type.Equals("dword", StringComparison.OrdinalIgnoreCase))
        {
            return 2;
        }
