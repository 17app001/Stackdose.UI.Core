...
public static class ShellMonitorAddressBuilder
{
    private static readonly Regex AddressPattern = new("^([A-Za-z]+)(\\d+)$", RegexOptions.Compiled);

    public static string Build(IEnumerable<ShellMachineConfig> configs)
    {
        var addresses = CollectReadableAddresses(configs);
    ...
    public static List<string> CollectReadableAddresses(IEnumerable<ShellMachineConfig> configs)
    {
        return configs
    ...
    private static IEnumerable<string> ExpandAddresses(ShellTagConfig tag)
    {
