...
| 項目 | 狀態 | 缺口說明 |
|---|---|---|
| **AlarmViewer** | 底層完整，上層缺 | 需手寫 alarms.json（格式：device/bit/group/description），MachinePageDesigner 無視覺化編輯 UI |
| **SensorViewer** | 架構有，實作待驗 | sensors.json 格式未確認，同 AlarmViewer 無設定 UI |
| **SecuredButton** | 僅 write-1 | 缺 writeValue / pulse（寫1→延遲→寫0）/ toggle / 多步驟 |
| **工程師複雜框架** | DeviceFramework 有 | DesignPlayer 路線無 JSON 多步驟指令，複雜邏輯需走 DeviceFramework |

**alarms.json 格式（目前需手寫）：**
```json
{ "alarms": [
