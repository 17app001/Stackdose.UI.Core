...
namespace Stackdose.UI.Core.Helpers
{
    ...
    public static class ThemeManager
    {
        ...
        // Light color override dictionaries added to Application.Resources top-level.
        // Adding at the end of MergedDictionaries makes them take priority over the nested
        // Colors.xaml inside Theme.xaml, and the top-level change triggers DynamicResource updates.
        private static ResourceDictionary? _lightCoreOverride;
        private static ResourceDictionary? _lightTemplatesOverride;

        private static void SwapColorDictionaries(bool isLight)
        {
            if (Application.Current == null) return;
            var merged = Application.Current.Resources.MergedDictionaries;

            if (_lightCoreOverride != null)
            {
                merged.Remove(_lightCoreOverride);
                _lightCoreOverride = null;
            }
            if (_lightTemplatesOverride != null)
            {
                merged.Remove(_lightTemplatesOverride);
                _lightTemplatesOverride = null;
            }

            if (isLight)
            {
                _lightCoreOverride = new ResourceDictionary
                {
                    Source = new Uri("pack://application:,,,/Stackdose.UI.Core;component/Themes/LightColors.xaml")
                };
                merged.Add(_lightCoreOverride);

                try
                {
                    _lightTemplatesOverride = new ResourceDictionary
                    {
                        Source = new Uri("pack://application:,,,/Stackdose.UI.Templates;component/Resources/LightCommonColors.xaml")
                    };
                    merged.Add(_lightTemplatesOverride);
                }
                catch
                {
                    _lightTemplatesOverride = null;
            ...
            Color? foregroundColor = null)
        {
            lock (_lock)
            {
                try
                {
                    ...
                    SwapColorDictionaries(isLightTheme);
