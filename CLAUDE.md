...
| `Stackdose.App.DesignRuntime` | `./Stackdose.App.DesignRuntime/` | 真實PLC連線 + JSON載入執行專案 | ✅ 完整 |
| `Stackdose.App.DesignPlayer` | `./Stackdose.App.DesignPlayer/` | 可交付量產 Shell App，JSON 驅動 + PLC 連線 + 登入管控 | ✅ 完整 |

### 工具
| 專案 | 路徑 | 說明 | 狀態 |
|---|---|---|---|
| `Stackdose.Tools.MachinePageDesigner` | `./Stackdose.Tools.MachinePageDesigner/` | 自由畫布拖曳頁面設計器（輸出 .machinedesign.json） | ✅ 完整 |
| `Stackdose.Tools.DesignViewer` | `./Stackdose.Tools.DesignViewer/` | 拖入 JSON 即時預覽畫布 | ✅ 完整 |
...
- **PLC Tags（地址名稱對照表）** — 工程師在設計器 Toolbar 點「📌 Tags」定義地址→名稱；設計師在所有 PLC 位址欄位可下拉選取，ToString()=Address 確保填入的是地址而非名稱。JSON 存於 `document.tags[]` 欄位。

### 已完成 — 架構優化（P0/P1/P2，共 13 項）
- **P0: SecurityContext 非同步化** — `Login` → `LoginAsync`，消除 `.Result` UI 阻塞；LoginDialog 直接 await
- **P0: ComplianceContext 強化** — 新增 `IsInitialized` 旗標 + EventLog 失敗寫入
- **P1: BaseRuntimeControlFactory** — DesignPlayer / DesignRuntime 共用 ~300 行工廠程式碼，各自精簡至 14 行
- **P1: 共用 PLC 輪詢 Timer** — BitIndicator N 個 DispatcherTimer → 1 個 500ms 共用 Timer
- **P1: 日誌匯出實作** — LogManagementPanel 新增 Operation / Event / PeriodicData 三組 PDF 匯出
- **P2: ViewModel 精簡** — DesignerItemViewModel `CommitStr`/`CommitDbl` 輔助，543→467 行（-14%）
- **P2: ConfigureAwait(false)** — Services/Helpers 58 處非 UI async 呼叫加入 ConfigureAwait(false)
- **P2: 測試 SQLite 對齊** — Microsoft.Data.Sqlite 9.0.9 → 10.0.1（與主專案一致）
...
| `docs/kb/designer-quickstart.md` | **設計師 Quick Start**（角色分工、MachinePageDesigner → DesignPlayer 完整流程） |
